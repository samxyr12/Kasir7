...
// Create a nominal button for quick cash payment selection
function createNominalButton(nominal, container) {
    const button = document.createElement('button');
    button.className = 'nominal-button bg-gray-100 p-2 rounded-md hover:bg-gray-200 transition duration-200 text-sm font-medium';
    button.textContent = formatRupiah(nominal, '', false);
    button.onclick = () => {
        const nominalInput = document.getElementById('nominalCash');
        if (nominalInput) {
            nominalInput.value = formatRupiah(nominal, '', false);
            updateKembalian();
        } else {
            console.error('Nominal input element not found');
        }
    };
    if (container) {
...
// Generate default nominal values for cash payments
function generateDefaultNominals(total) {
    const defaultNominals = [];
    const roundedTotal = Math.ceil(total / 1000) * 1000; // Round up to nearest thousand
    
    // Default nominals: total, +1000, +2000, +5000
...
function generateAnalysisNominals(total) {
    ...
    // Round up to nearest five thousand
    ...
    // Psychological nominal (slightly above total)
...
// Toggle between cash and QRIS payment methods, generating nominal buttons for cash
function pilihMetode(metode) {
    ...
    if (metodeCash && metodeQRIS) {
        if (metode === 'cash') {
            ...
            // Get final total from totalBayar or appliedVoucher to include voucher discount
            const totalElement = document.getElementById('totalBayar');
            let total = totalElement ? parseRupiahToNumber(totalElement.textContent) : 0;
            
            // Use appliedVoucher.finalTotal if available
            const appliedVoucher = JSON.parse(localStorage.getItem('appliedVoucher'));
            if (appliedVoucher && appliedVoucher.finalTotal !== undefined) {
                total = appliedVoucher.finalTotal;
                // Ensure totalBayar reflects the discounted total
                if (totalElement) totalElement.textContent = formatRupiah(total, '', false);
            }
            
            // Validate total
            if (total < 0) {
                console.error('Total cannot be negative:', total);
                total = 0;
            }
            ...
            if (defaultContainer && analysisContainer) {
                // Clear containers
                defaultContainer.innerHTML = '';
                analysisContainer.innerHTML = '';
                
                // Generate and display default nominals
                ...
                // Generate and display analysis nominals
                ...
                console.error('Nominal containers (defaultNominals or analysisNominals) not found');
            }
        ...
        console.error('Payment method elements (metodeCash or metodeQRIS) not found');
    }
}

// Update the displayed change based on cash input and discounted total
function updateKembalian() {
    const nominalInput = document.getElementById('nominalCash');
    const totalElement = document.getElementById('totalBayar');
    const kembalianElement = document.getElementById('kembalian');
    const teksNominal = document.getElementById('teksNominal');

    if (nominalInput && totalElement && kembalianElement) {
        // Parse values, handling potential formatting issues
        const nominal = parseRupiahToNumber(nominalInput.value) || 0;
        let total = parseRupiahToNumber(totalElement.textContent) || 0;
        ...
        if (appliedVoucher && appliedVoucher.finalTotal !== undefined) {
            total = appliedVoucher.finalTotal;
            // Ensure totalBayar reflects the discounted total
            totalElement.textContent = formatRupiah(total, '', false);
        }
        ...
        // Update teksNominal for user feedback
        if (teksNominal) {
            teksNominal.textContent = nominal > 0 ? `Nominal: ${formatRupiah(nominal)}` : '';
            teksNominal.style.color = kembalian < 0 ? '#ef4444' : '#6b7280';
            if (kembalian < 0) {
                teksNominal.textContent += ` (Kurang: ${formatRupiah(Math.abs(kembalian))})`;
            }
        }
    } else {
        console.error('Required elements for kembalian update (nominalCash, totalBayar, or kembalian) not found');
    }
}
...
function prosesPembayaran(metode) {
    ...
    // Use appliedVoucher.finalTotal to ensure discounted total
    const appliedVoucher = JSON.parse(localStorage.getItem('appliedVoucher'));
    let discount = 0;
    if (appliedVoucher && appliedVoucher.finalTotal !== undefined) {
        total = appliedVoucher.finalTotal;
        discount = appliedVoucher.discount;
        // Sync totalBayar with discounted total
        if (totalElement) totalElement.textContent = formatRupiah(total, '', false);
    }
    ...
    if (metode === 'cash') {
        ...
        if (nominal < total) {
            Swal.fire({
                ...
                html: `Total yang harus dibayar: ${formatRupiah(total)}<br>Kurang: ${formatRupiah(total - nominal)}`,
                confirmButtonColor: '#ff6b6b',
            });
            return;
        }
        ...
        simpanTransaksi(metode, total, kembalian, idTransaksi, keranjang, memberTransaksi?.id, poinDiperoleh, nominal, appliedVoucher);

        if (memberTransaksi) {
            generateAutomaticVoucher(total, keranjang, memberTransaksi);
        ...
        const kembalianElement = document.getElementById('kembalian');
        if (kembalianElement) kembalianElement.textContent = '0';
        const teksNominal = document.getElementById('teksNominal');
        if (teksNominal) teksNominal.textContent = '';
        document.getElementById('popup').style.display = 'none';
        localStorage.removeItem('appliedVoucher');
        const kodePromosiInput = document.getElementById('kodePromosiInput');
        if (kodePromosiInput) kodePromosiInput.value = '';
        const voucherStatus = document.getElementById('voucherStatus');
        if (voucherStatus) voucherStatus.innerHTML = '';
        ...
        Swal.fire({
            ...
            html: `Total: ${formatRupiah(total)}<br>${discount > 0 ? `Diskon: ${formatRupiah(discount)}<br>` : ''}Kembalian: ${formatRupiah(kembalian)}`,
        ...
        simpanTransaksi(metode, total, 0, idTransaksi, keranjang, memberTransaksi?.id, poinDiperoleh, total, appliedVoucher);
        ...
        Swal.fire({
            ...
            html: `Total Pembayaran: ${formatRupiah(total)}<br>${discount > 0 ? `Diskon: ${formatRupiah(discount)}` : ''}`,
            confirmButtonColor: '#4caf50',
        }).then(() => {
            cetakStruk(idTransaksi);
        });
...
function simpanTransaksi(metode, total, kembalian, idTransaksi, keranjang, memberId = null, poin = 0, nominal = 0, voucher = null) {
    ...
    keranjang.forEach((item) => {
        const transactionEntry = {
            ...
            kembalian: kembalian,
            metode: metode,
            diskon: voucher ? voucher.discount : (item.potongan || 0), // Voucher discount or item-specific discount
            persenDiskon: voucher && voucher.type === 'percentage' ? voucher.value : (item.potongan || 0), // Percentage discount if applicable
            tanggal: tanggal, // YYYY-MM-DD
            memberId: memberId,
            poin: poin, // Points earned
            voucherCode: voucher ? voucher.code : null // Voucher code if applied
        };
    ...
    try {
        ...
        if (voucher) {
            ...
            const voucherIndex = vouchers.findIndex(v => v.code === voucher.code);
            if (voucherIndex !== -1) {
        ...
        try {
            ...
            // Mark voucher as used in fallback case
            if (voucher) {
                let vouchers = JSON.parse(localStorage.getItem('vouchers')) || [];
                const voucherIndex = vouchers.findIndex(v => v.code === voucher.code);
                if (voucherIndex !== -1) {
...
function generateVoucherCode() {
    return 'VCHR' + Math.random().toString(36).substr(2, 8).toUpperCase();
}

// Existing generateAutomaticVoucher function
function generateAutomaticVoucher(total, keranjang, member) {
    let conditions = JSON.parse(localStorage.getItem('voucherConditions')) || [];
    conditions.forEach(condition => {
        let shouldGenerate = false;
        if (condition.type === 'minPurchase' && total >= condition.minPurchase) {
            shouldGenerate = true;
        }
        if (condition.type === 'specificItem' && condition.specificItem && condition.minQuantity) {
            const item = keranjang.find(i => i.kode === condition.specificItem);
            if (item && item.jumlah >= condition.minQuantity) {
                shouldGenerate = true;
            }
        }
        if (shouldGenerate) {
            const expiryDate = new Date();
            expiryDate.setDate(expiryDate.getDate() + condition.voucher.expiryDays);
            const newVoucher = {
                code: generateVoucherCode(), // Uses the function here
                type: condition.voucher.type,
                value: condition.voucher.value,
                minPurchase: condition.voucher.minPurchase,
                specificItem: null,
                minQuantity: 0,
            ...
            Swal.fire({
                icon: 'success',
                title: 'Voucher Baru!',
                text: `Selamat! Anda mendapatkan voucher ${newVoucher.type === 'percentage' ? newVoucher.value + '%' : formatRupiah(newVoucher.value)} dengan kode ${newVoucher.code}`,
                confirmButtonColor: '#4caf50'
            });
        }
    });
...
function applyVoucher() {
    const voucherCode = document.getElementById('kodePromosiInput').value;
    const keranjang = JSON.parse(localStorage.getItem('keranjang')) || [];
    const totalAkhir = keranjang.reduce((total, item) => total + item.total, 0);
    let vouchers = JSON.parse(localStorage.getItem('vouchers')) || [];
    const voucherStatus = document.getElementById('voucherStatus');
    const memberTransaksi = JSON.parse(localStorage.getItem('memberTransaksi'));

    // Find valid voucher (ensure it belongs to the member or is not member-specific)
    const voucher = vouchers.find(v => 
        v.code === voucherCode && 
        v.status === 'active' && 
        new Date(v.expiryDate) >= new Date() &&
        (v.memberId === memberTransaksi?.id || !v.memberId)
    );
    
    if (!voucher) {
        voucherStatus.innerHTML = '<span style="color: red;">Voucher tidak valid, telah kadaluarsa, atau tidak sesuai dengan member!</span>';
        return false;
    }

    // Validate voucher conditions
    let isValid = true;
    if (voucher.minPurchase > totalAkhir) {
        isValid = false;
        voucherStatus.innerHTML = `<span style="color: red;">Minimal pembelian ${formatRupiah(voucher.minPurchase)} belum tercapai!</span>`;
    }

    if (voucher.specificItem) {
        const item = keranjang.find(i => i.kode === voucher.specificItem);
        if (!item || (voucher.minQuantity && item.jumlah < voucher.minQuantity)) {
            isValid = false;
            voucherStatus.innerHTML = `<span style="color: red;">Syarat item ${voucher.specificItem} dengan jumlah minimum ${voucher.minQuantity} belum terpenuhi!</span>`;
        }
    }
    ...
    if (isValid) {
        ...
        if (voucher.type === 'percentage') {
            discount = totalAkhir * (voucher.value / 100);
        } else {
        ...
        voucherStatus.innerHTML = `<span style="color: green;">Voucher berhasil diterapkan! Diskon: ${formatRupiah(discount)}</span>`;
        
        // Update displayed total
        const totalBayarElement = document.getElementById('totalBayar');
        const totalBayarQRISElement = document.getElementById('totalBayarQRIS');
        if (totalBayarElement) totalBayarElement.innerText = formatRupiah(totalAkhir - discount, '', false);
        if (totalBayarQRISElement) totalBayarQRISElement.innerText = formatRupiah(totalAkhir - discount, '', false);
...
document.getElementById('terapkanVoucher').addEventListener('click', applyVoucher);
