...
function simpanTransaksi(metode, total, kembalian, idTransaksi, keranjang, memberId = null, poin = 0, nominal = 0) {
    ...
    // Coba untuk mengambil data transaksi dari local storage
    try {
        const storedTransaksi = localStorage.getItem('transaksi');
        
        // Jika ada data, coba untuk menguraikannya
        if (storedTransaksi) {
        ...
        if (!Array.isArray(transaksi)) {
            throw new Error("Data transaksi tidak valid, menginisialisasi ulang.");
        }
    } catch (error) {
        console.error("Error retrieving transaksi:", error);
        // Jika terjadi kesalahan, inisialisasi transaksi sebagai array kosong
        transaksi = [];
    }
    ...
    // Iterasi melalui keranjang dan simpan setiap item ke dalam transaksi
    ...
    // Simpan transaksi berdasarkan metode
    if (metode === 'cash') {
        localStorage.setItem('transaksi_cash', JSON.stringify(transaksi));
    } else if (metode === 'qris') {
        localStorage.setItem('transaksi_qris', JSON.stringify(transaksi));
    }
...
function kirimStruk() {
    const memberTransaksi = JSON.parse(localStorage.getItem('memberTransaksi'));
    const struk = buatStrukWhatsApp(); // Ambil struk yang sudah diformat

    if (memberTransaksi && memberTransaksi.noTelp) {
        // Jika menggunakan member dan nomor telepon ada
        const nomorMember = memberTransaksi.noTelp.replace(/\D/g, ''); // Hapus karakter non-digit
        kirimStrukKeWhatsApp(nomorMember); // Kirim struk ke nomor member
    } else {
        // Jika tidak menggunakan member, tampilkan input untuk nomor WhatsApp
        tanyaNomorWA();
    }
}

// Fungsi untuk menampilkan dialog input nomor WhatsApp manual
