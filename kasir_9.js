    ...
    if (transaksiTerpilih.length > 0) {
        ...
        function buatStrukWhatsApp() {
            ...
            if (transaksiTerpilih[0].memberId) {
                ...
                if (member) {
                    struk.push(`Member      : ${member.nama}`);
                    struk.push(`ID Member   : ${member.id}`);
                }
            }
            ...
            struk.push(`Metode Bayar:       ${transaksiTerpilih[0].metode.toUpperCase()}`);
            ...
            if (transaksiTerpilih[0].memberId && transaksiTerpilih[0].poin > 0) {
                struk.push('--------------------------------');
                struk.push(`Poin Didapat:       +${transaksiTerpilih[0].poin} poin`);
                
                // Cari total poin member saat ini
                const members = JSON.parse(localStorage.getItem('members')) || [];
                const member = members.find(m => m.id === transaksiTerpilih[0].memberId);
                if (member) {
                    struk.push(`Total Poin  :        ${member.poin} poin`);
                }
            }
...
function voidBarang(index) {
    ...
    Swal.fire({
        title: 'Masukkan PIN',
        input: 'password',
        inputPlaceholder: 'Masukkan PIN untuk void',
        inputAttributes: {
            autocapitalize: 'off',
            autocorrect: 'off'
        },
        ...
        // Check if user clicked confirm and entered the correct PIN
        if (pinResult.isConfirmed) {
            ...
            if (pin !== '451') {
                Swal.fire({
                    icon: 'error',
                    title: 'PIN Salah',
                    text: 'PIN yang Anda masukkan tidak valid.',
                    confirmButtonColor: '#e74c3c'
                });
                return;
            }
            
            // PIN is correct, get the item details
            let keranjang = JSON.parse(localStorage.getItem('keranjang')) || [];
            ...
            if (!item) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Item tidak ditemukan dalam keranjang',
                    confirmButtonColor: '#e74c3c'
                });
                return;
            }
            ...
            if (isInOpname) {
                Swal.fire({
                    icon: 'error',
                    title: 'Void Tidak Dapat Dilakukan',
                    text: 'Barang ini sedang dalam proses stok opname dan tidak dapat di-void.',
                    confirmButtonColor: '#e74c3c'
                });
                return;
            }
            
            // Format the price and total for display
            const formattedHarga = formatRupiah(item.harga);
            const formattedTotal = formatRupiah(item.harga * item.jumlah);
            
            // Show item details and ask for void reason
            Swal.fire({
                title: 'Void Barang',
                html: `
                    <div style="text-align: left; margin-bottom: 20px;">
                        <div style="margin-bottom: 10px;">
                            <strong style="display: inline-block; width: 100px;">Nama:</strong> 
                            <span style="color: #e74c3c; font-weight: 600;">${item.nama}</span>
                        </div>
                        <div style="margin-bottom: 10px;">
                            <strong style="display: inline-block; width: 100px;">Jumlah:</strong> 
                            <span style="color: #e74c3c; font-weight: 600;">${item.jumlah}</span>
                        </div>
                        <div style="margin-bottom: 10px;">
                            <strong style="display: inline-block; width: 100px;">Harga:</strong> 
                            <span style="color: #e74c3c; font-weight: 600;">${formattedHarga}</span>
                        </div>
                        <div style="margin-bottom: 20px;">
                            <strong style="display: inline-block; width: 100px;">Total:</strong> 
                            <span style="color: #e74c3c; font-weight: 600;">${formattedTotal}</span>
                        </div>
                        <div style="margin-top: 15px;">
                            <label for="swal-alasan-void" style="font-weight: 600; display: block; margin-bottom: 5px;">Alasan Void:</label>
                            <select id="swal-alasan-void" class="swal2-input" style="width: 100%; margin: 0;">
                        ...
                        <div id="swal-alasan-lain-container" style="display: none; margin-top: 10px;">
                            <label for="swal-alasan-lain" style="font-weight: 600; display: block; margin-bottom: 5px;">Alasan Lainnya:</label>
                            <input id="swal-alasan-lain" class="swal2-input" placeholder="Masukkan alasan void..." style="width: 100%; margin: 0;">
                        </div>
                    </div>
                `,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Void Barang',
                cancelButtonText: 'Batal',
                confirmButtonColor: '#e74c3c',
                cancelButtonColor: '#7f8c8d',
                didOpen: () => {
                    // Add event listener to the alasan dropdown
                ...
                if (voidResult.isConfirmed) {
                    ...
                    // If neither element exists, it means the modal was closed unexpectedly
                    if (!alasanSelect) {
                        console.error('Could not find alasan select element');
                        return;
                    }
                    
                    let alasan = alasanSelect.value;
                    
                    // If "Lainnya" is selected, get custom reason
                    if (alasan === 'Lainnya' && alasanLain) {
                        if (!alasanLain.value.trim()) {
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: 'Alasan void harus diisi',
                                confirmButtonColor: '#e74c3c'
                            });
                            return;
                        }
...
function processVoid(index, alasan) {
    try {
        // Get current cart, stock and void records
        let keranjang = JSON.parse(localStorage.getItem('keranjang')) || [];
        let barang = JSON.parse(localStorage.getItem('barang')) || [];
        ...
        // Generate void code and current timestamp
        const kodeVoid = generateKodeVoid();
        const now = new Date();
        const tanggal = now.toISOString();
        
        // Get current user/kasir information if available
        const username = localStorage.getItem('username') || 'admin';
        
        // Create void record with complete information
        voids.push({
            kodeVoid: kodeVoid,
            namaBarang: item.nama,
            jumlah: item.jumlah,
        ...
        if (barangItem) {
            ...
            if (barangItem.terjual < 0) barangItem.terjual = 0; // Prevent negative values
            localStorage.setItem('barang', JSON.stringify(barang));
        }
        ...
        Swal.fire({
            ...
            html: `
                <div>
                    <p>Barang berhasil di-void dengan kode: <strong>${kodeVoid}</strong></p>
                    <p style="margin-top: 10px; color: #7f8c8d; font-size: 14px;">Alasan void: ${alasan}</p>
                </div>
            `,
        ...
        console.error('Error in processVoid:', error);
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Terjadi kesalahan saat memproses void: ' + error.message,
            confirmButtonColor: '#e74c3c'
        });
    }
}
