...
function downloadExcel() {
    try {
        ...
        // Ambil seluruh data stok opname (semua status)
        ...
        // --- Sheet 1: Data Barang ---
        let barangSheetData = [
            ['Kategori', 'Kode Toko', 'Kode Barang', 'PLU', 'Nama Barang', 'Harga Beli', 'Harga Jual', 
             'Stok', 'Total Terjual', 'Keuntungan', 'Persentase Terjual', 'Target Penjualan', 
             'Persentase Target', 'Diskon Aktif', 'Harga Setelah Diskon']
        ];
        ...
        let summarySheetData = [
            ['Kategori', 'Total Barang', 'Total Stok', 'Total Terjual', 'Total Keuntungan', 
        ...
        urutanKategori.forEach(kategori => {
            ...
            if (items && items.length > 0) {
                ...
                const totalStok = items.reduce((sum, item) => sum + (parseInt(item.stok) || 0), 0);
                ...
                const avgPersentaseTerjual = totalStok > 0 
                    ? ((totalTerjual / totalStok) * 100).toFixed(2) 
                    : '0';
        ...
        const overallStok = barangData.reduce((sum, item) => sum + (parseInt(item.stok) || 0), 0);
        ...
        const overallAvgPersentaseTerjual = overallStok > 0 
            ? ((overallTerjual / overallStok) * 100).toFixed(2) 
            : '0';
        ...
        // --- Sheet 6: Riwayat Stock Opname (Detail) ---
        let riwayatStockOpnameSheetData = [
            ['No Opname', 'Tanggal', 'Petugas', 'Tipe Opname', 'Kategori', 'Status', 'Catatan', 
             'Kode Barang', 'PLU', 'Nama Barang', 'Kode Toko', 'Kategori Barang', 'Stok Sistem', 
             'Stok Aktual', 'Selisih', 'Selisih Rupiah', 'Harga Beli', 'Keterangan']
        ];

        // Proses semua entri stok opname
        stokOpnameData.forEach(opname => {
            ...
            // Proses setiap item dalam opname
        ...
        let summaryStockOpnameSheetData = [
            ['Kategori', 'Total Sesi Opname', 'Total Barang Unik', 'Total Selisih Stok', 
             'Total Selisih Rupiah', 'Selisih Positif', 'Selisih Negatif']
        ];
        ...
        urutanKategori.forEach(kategori => {
            ...
            if (opnameItems && opnameItems.length > 0) {
                const totalSesiOpname = [...new Set(opnameItems.map(op => op.noOpname))].length;
        ...
        // Add overall summary for stock opname
        const totalSesiOpname = [...new Set(stokOpnameData.map(op => op.noOpname))].length;
        ...
        Swal.fire({
            ...
            html: `
                <div style="text-align: left;">
                    ...
                    <ul>
                        <li>Data Barang: ${barangData.length} item</li>
                        ...
                        <li>Summary Stock Opname: ${summaryStockOpnameSheetData.length - 1} kategori</li>
                    </ul>
            ...
            if (result.isConfirmed) {
                ...
                XLSX.writeFile(workbook, `inventori_lengkap_${new Date().toLocaleDateString('id-ID').replace(/\//g, '-')}.xlsx`);
                ...
                const zip = new JSZip();
                const filename = `inventori_lengkap_${new Date().toLocaleDateString('id-ID').replace(/\//g, '-')}.xlsx`;
                ...
                zip.generateAsync({ type: 'blob' }).then(function(content) {
                    ...
                    link.download = `inventori_lengkap_${new Date().toLocaleDateString('id-ID').replace(/\//g, '-')}.zip`;
                    link.click();
